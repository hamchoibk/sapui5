...
tcbmwApp.controller('askwalletAcceptConfirmDoneController',['$scope','$state','TransactionsFactory', 'tcbTransferFactory','$rootScope',function ($scope,$state,TransactionsFactory,tcbTransferFactory,$rootScope) {
    if ( $state.current.name == 'askwalletAcceptConfirmDone'){
        var baseStatus = {
            isClose:true,
            onAction: function() {
    ...
    var transaction = TransactionsFactory.getSelectedSocialTransaction();
    console.log("transaction", transaction);
    transaction.status = "Accepted";
    $scope.fee = session.transactionFee;

    $scope.transaction = transaction;
    $scope.transactionIcon = "none";
