'use strict';
tcbmwApp.controller('socialListController',['$scope','$state','$filter','friendFactory','FavouritesFactory','SocialFactory','$rootScope',function($scope,$state,$filter,friendFactory,FavouritesFactory,SocialFactory,$rootScope){
    ...
    var baseStatus = {
        isBack: true,
        onBack: function() {
            $state.go(friendFactory.getRequestMoneyType());
    ...
    var successCallback = function(success){
        ...
        var lastSource = friendFactory.getRequestMoneyType();

        if (friendObject.mobileNumber == session.msisdn) {
            MessageBox("","SEND_MONEY_EWALLET_ERROR_YOURSELF");
            friendFactory.resetFriendUser();
            console.log("friendFactory.getRequestMoneyType() == ", friendFactory.getRequestMoneyType());
        } else {
        ...
        if(lastSource=='sendMoney')
            $state.go('sendMoney');
        else if(lastSource=='dashboard.socialindex')
    ...
    $scope.selectFavourite=function(favourite){
        var friendObj;
        console.log(favourite.recentChannel);
        ...
        if(friendFactory.getRequestMoneyType()=='sendMoney')
           $state.go('sendMoney');
        else if(friendFactory.getRequestMoneyType()=='dashboard.socialindex')
           $state.go('dashboard.friendcontactdetail');
    }
    ...
    var googlePlusCallback = function(success){
        MessageBox(success);
    };
